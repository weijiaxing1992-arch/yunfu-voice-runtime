...
func (s *Server) putAdminConfig(w http.ResponseWriter, r *http.Request) {
	...
	s.adminTransaction(func() adminResponse {
		...
		if body.Config.Media.Binary != s.admin.active.Media.Binary || body.Config.Journal.Path != s.admin.active.Journal.Path || body.Config.Media.PlaybackRoot != s.admin.active.Media.PlaybackRoot || !sameDialplan(body.Config.SIP.Dialplan, s.admin.active.SIP.Dialplan) || !samePCMStream(body.Config.PCMStream, s.admin.active.PCMStream) {
			return adminFailure(422, errors.New("程序、日志、放音根目录、XML拨号计划和PCM流入口由部署启动文件管理，页面中只读"))
		}
