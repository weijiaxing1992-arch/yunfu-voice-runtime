...
候选增量源文档：[ASR 上行内部接口](asr-stream-reference.md) 描述内部 Go/ASR1 代理、资源状态及本批限定验收；尚未完成本轮文档包生成和部署，不并入下述历史 1.13 发布结论，也不新增公开 ASR HTTP 接口或原版兼容绿色。
...
15. [候选 ASR 上行内部接口](asr-stream-reference.md)：部署配置、Go方法、ASR1字段与期限、资源指标、FreeSWITCH边界；真实模型与当前整体发布仍待独立验收。
