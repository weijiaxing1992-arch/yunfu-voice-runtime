...
/* 识别资源独立展示，活动槽包含等待最终结果，不能当成识别成功数。旧服务缺字段明确显示未知。 */
function asrPanel(data) {
  if (!data) return panel('语音识别连接','当前服务版本未提供识别资源数据','<p class="field-help">更新后的服务可提供连接额度、起建和清理状态。</p>');
  if (!data.enabled) return panel('语音识别连接','本次部署未启用识别代理','<p class="field-help">在部署配置中启用固定识别代理后，可在此查看资源占用。</p>');
  const status=data.stopping ? badge('正在停止','warning') : badge('已启用','good');
  return panel('语音识别连接','主服务的识别连接和资源额度',`${healthRow('识别代理状态',status,'layers')}${healthRow('占用 / 上限',`${number(data.slots)} / ${number(data.max_streams)}`,'cpu')}${healthRow('正在建立 / 工作中',`${number(data.starting)} / ${number(data.active)}`,'phone')}${healthRow('等待资源清理',number(data.cleanup_pending),'clock')}${healthRow('累计额度拒绝 / 重复启动',`${number(data.rejected_capacity)} / ${number(data.rejected_uuid)}`,'shield')}<p class="field-help">工作中包括等待最终结果的连接；清理未确认的连接继续占用额度。以上均为资源计数，不代表识别成功数。</p>`);
}
