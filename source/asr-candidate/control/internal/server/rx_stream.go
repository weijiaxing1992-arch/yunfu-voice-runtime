...
type RXHandle struct {
	...
	noBindingConfirmed                  atomic.Bool // 仅实际订阅执行返回nil或确定未提交时置位；取消/普通retire不构成证明。
	// 生命期与收音读权分开：主动退订可继续等待识别最终结果，通话结束则必须立即撤销结果权。
	lifetimeOnce   sync.Once
	lifetime       context.Context
	cancelLifetime context.CancelFunc
}

var rxClosedLifetime = func() <-chan struct{} {
	done := make(chan struct{})
	close(done)
	return done
}()

// initLifetime只在订阅控制阶段分配一次子上下文；不在逐音频帧路径创建协程或取消桥接。
// 父服务结束会直接关闭Done，手动结束会解除父子引用，避免历史句柄积累到服务退出。
func (h *RXHandle) initLifetime() {
	h.lifetimeOnce.Do(func() {
		parent := h.service
		if parent == nil {
			parent = context.Background()
		}
		h.lifetime, h.cancelLifetime = context.WithCancel(parent)
		if h.service == nil || h.retired.Load() {
			h.cancelLifetime()
		}
	})
}

// LifetimeDone是通话授权的终止信号，不是输入结束信号。正常Unsubscribe/未来ASR Finish
// 只结束收音，可继续等待final/DONE；真实通话挂断、替换、代次失效或服务关闭才关闭本信号。
// 同一旧句柄永远返回同一Done，未知受理或迟到绑定不能重新创建生命期。
func (h *RXHandle) LifetimeDone() <-chan struct{} {
	if h == nil {
		return rxClosedLifetime
	}
	h.initLifetime()
	if h.Retired() {
		h.cancelLifetime()
...
// resolveBinding 在一次真实订阅执行结束后记录其资源归属。Pool契约的nil表示没有底层资源遗留，
// 必须与“仍等回执，binding尚为nil”区别开；调用方先超时拿到的unknown句柄也能最终收敛。
func (h *RXHandle) resolveBinding(sub rxSubscription) {
	if sub != nil {
		h.bind(sub)
		return
	}
	h.confirmNotSubmitted()
}

// confirmNotSubmitted 只用于已确证没有提交任务或完整执行返回nil的路径；不从ctx或退休标记推断。
func (h *RXHandle) confirmNotSubmitted() {
	if h != nil && h.binding.Load() == nil {
		h.noBindingConfirmed.Store(true)
	}
}
...
func (s *Server) handleRXSubscribe(r *rxSubscribeRequest) {
	...
	if r.delivery.Load() != 0 || r.ctx.Err() != nil {
		h.confirmNotSubmitted() // 发布后取消仍可能把句柄交给调用方，但此分支尚未提交任何job。
		h.retire()
		fail(r.ctx.Err())
		return
	}
	...
	default:
		c.rxPending = nil
		e.pending--
		h.confirmNotSubmitted() // 有界jobs队列拒绝，媒体执行者不可能再取得本任务。
		h.retire()
		fail(ErrRXHandleBusy)
	}
}
func (s *Server) handleRXSubscribeResult(result rxSubscribeResult) {
	...
	c.rxPending = nil
	// 回执等待期间也可能已结束通话或失去原代次；未知结果保留清理地址，不恢复结果授权。
	if c.Ended || !c.Allocated || c.Releasing || c.Worker != h.worker || c.Generation != h.generation {
		h.endLifetime()
	} else if p, ok := e.workerSnapshot(h.worker); !ok || !p.Healthy || p.PID <= 0 || p.Generation != h.generation || p.WorkerID != h.worker {
		h.endLifetime()
	}
	sub := h.binding.Load()
	if sub == nil {
	...
	for _, h := range []*RXHandle{c.rxHandle, c.rxPending} {
		if h != nil {
			if actualRelease {
				...
				// 此处是Server.finish中的真实通话结束，不是未来ASR的主动Finish。
				h.endLifetime()
				h.revoke()
			}
		}
